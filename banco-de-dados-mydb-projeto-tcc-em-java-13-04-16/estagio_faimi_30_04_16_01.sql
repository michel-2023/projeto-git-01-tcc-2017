...
CREATE TABLE IF NOT EXISTS `estagio_faimi`.`cad_emp_faimi` (
  ...
  `tipo_pessoa` VARCHAR(45) NOT NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `estagio_faimi`.`cad_enc_faimi` (
  `id_encaminhamento` INT(11) NOT NULL AUTO_INCREMENT COMMENT '',
  `data_enc` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '',
  `data_encaminhamento` VARCHAR(45) NOT NULL COMMENT '',
  `data_inicio` VARCHAR(45) NULL DEFAULT NULL COMMENT '',
  `contratado` VARCHAR(250) NOT NULL COMMENT '',
  `conceito` VARCHAR(500) NULL DEFAULT NULL COMMENT '',
  `fkVaga` INT(11) NOT NULL COMMENT '',
  `fkAluno` INT(11) NOT NULL COMMENT '',
  PRIMARY KEY (`id_encaminhamento`)  COMMENT '',
  INDEX `fk_cad_enc_faimi_cad_vaga_faimi1_idx` (`fkVaga` ASC)  COMMENT '',
  INDEX `fk_cad_enc_faimi_cad_aluno_faimi1_idx` (`fkAluno` ASC)  COMMENT '',
  CONSTRAINT `fk_cad_enc_faimi_cad_vaga_faimi1`
    FOREIGN KEY (`fkVaga`)
    REFERENCES `estagio_faimi`.`cad_vaga_faimi` (`id_user_vaga`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_cad_enc_faimi_cad_aluno_faimi1`
    FOREIGN KEY (`fkAluno`)
